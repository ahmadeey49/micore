...
user_type = st.sidebar.selectbox("Applicant Category", ["Banked (With Statement)", "Unbanked (Manual)"])

income, expenses, loans = 0, 0, 0

if user_type == "Banked (With Statement)":
    input_method = st.radio("Choose Input Method", ["Upload Statement (PDF/Image)", "Enter Manually"])
...
amount_needed = st.number_input("Amount of Loan Needed (₦):", min_value=1000, step=1000)
...
if st.button("Analyze with Micore AI"):
    ...
    elif monthly_cashflow < monthly_repayment * 0.95:  # tolerance margin
        st.error("### RESULT: NOT ELIGIBLE ❌")
    ...
    ax.bar(['Monthly Income','Monthly Expenses'], [income, expenses], color=['#28a745','#dc3545'])
